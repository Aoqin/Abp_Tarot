...
namespace Acme.Tarot.Cards
{
  ...
  public class TarotCardSolutionService : ApplicationService
  {
    ...
    public async Task<List<TarotCardSolution>> GetAllTarotCardSolutionByTarotCardCollectionIdAsync(Guid id)
    {
      // var result = await TarotCardSolutions.GetQueryableAsync (true, x => x.TarotCardCollectionId == id);
      // var dbContext = await TarotCardSolutions.GetDbContextAsync<TarotCardSolution>();
      // var query = from solution in dbContext.Set<TarotCardSolution>()
      //             where solution.TarotCardCollectionId == id
      //             select solution;
      var result = await TarotCardSolutions.GetListAsync();
      // var result = query.ToList<TarotCardSolution>();
      return result;
    }
    ...
    public async Task<TarotCardSolution> GetAsyncWithDetails(Guid id, List<Guid> tarotCardIds)
    {
      ...
      if (result != null)
      {
        // TarotCardCollection = await TarotCardCollections.GetAsync(x => x.Id == id);
    ...
    /// 创建
    /// </summary>
    /// <param name="tarotCardSolutionCreateDto"></param>
    /// <returns></returns>
    ...
    /// 更新
    /// </summary>
    /// <param name="tarotCardSolutionUpdateDto"></param>
    /// <returns></returns>
